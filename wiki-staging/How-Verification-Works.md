...
| 2a. Card & anchors (v0.6) | The readable on-chain card (title, author, short description) matches the file byte-for-byte; the identification anchors match on-chain `anchorsHash` | Automatic on v0.6 registries — any mismatch is reported as TAMPERED, not a warning |
...
| 6. Physical seal (optional since v0.6) | The physical object is linked to the passport | Numbered seal (visual) or NFC chip (see [NFC Seals](NFC-Seals)) |
