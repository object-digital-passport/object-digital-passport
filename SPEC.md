...
| **Variant salt** *(v0.7)* | The per-unit secret that hides which variant a blind-box unit contains until it is opened (§20.4). It is carried **inside** the sealed package, never derivable from the unit key — those are two different secrets guarding two different things. *Avoid:* "reveal code", "variant key", conflating it with the **Unit Key**. |
