...
Every registered object receives a globally unique human-readable identifier.
...
- Human ID is immutable after minting
- Human ID is the canonical identifier for all lookups
...
or submitting a proof. Registration assigns a permanent globally unique Creator ID.

**Creator ID is mandatory.** The contract rejects any mint or proof transaction
...
by verifiers. Only the Creator ID is on-chain. Verifiers display only the ID —
...
**The protocol does not verify whether a Creator ID actually belongs to a claimed institution.**
Only the act of publicly publishing the Creator ID on a real website — combined with the
...
Generated using the same algorithm as Human ID:
...
allowing ODP Creator IDs to be embedded inside C2PA manifests.
