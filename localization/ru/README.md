...
**Линия 0.x** — это proof-of-concept: поведение и деплой могут меняться. Каждый адрес контракта — **отдельный** реестр; записи **не переносятся** между деплоями сами по себе. **Этот репозиторий описывает эталонную линию v0.3** (поколение реестра на цепочке **3**); в будущем стабильная **v1** может задать миграцию или двойное чтение — см. [`../../docs/VERSIONING_AND_RELEASES.md`](../../docs/VERSIONING_AND_RELEASES.md) и [`SPEC.md`](SPEC.md).
...
2. **Паспорт** — фиксируется запись паспорта: хэши содержимого, при необходимости URL и метаданные в реестре **v0.3** (и дополнительные обязательства по спецификации).
