...
1. **Кошелёк и самохранение.** Используйте отдельный кошелёк для экспериментов с ODP (не основной «копилочный»). Разберитесь, как устроен кошелёк, храните резервную фразу офлайн и относитесь к каждому сайту с запросом «подключить кошелёк» как к потенциально опасному. Официальные материалы: [руководство MetaMask](https://support.metamask.io/start/user-guide-new/). В сети Polygon комиссии снимаются в **POL**; **отдельного сбора протокола ODP нет** — см. [Сеть и стоимость](#сеть-и-стоимость).
...
4. Операторские заметки v0.3: [`../../RELEASE_v0.3.md`](../../RELEASE_v0.3.md). **Свой деплой:** [`../../deploy/README.md`](../../deploy/README.md).
...
*Информационный блок, не юридическая консультация.* Правила обращения с криптоактивами, доступ к международным сервисам, способы пополнения (on-ramp) и работа провайдеров могут меняться. Оценка рисков — на пользователе; полезно иметь **запасной план** доступа к средствам и к документации кошелька (официальные сайты [MetaMask](https://support.metamask.io/), при использовании — [Rabby](https://rabby.io/) и т.д.), не полагаясь только на ссылки из соцсетей — **фишинг** кошельков распространён.
...
### 4) Публикация `passport.json` (по желанию, но желательно)

- Разместите канонический JSON по `dataUrl` (**HTTPS**). При варианте «папка на хостинге» в эталонных примерах рядом лежит ZIP **`.odpass`**; проверка извлекает из него `passport.json`.
- После регистрации не меняйте сохранённые байты, если рассчитываете, что проверка хэшей продолжит совпадать.

### 5) Проверка

- Откройте [Verify](https://object-digital-passport.github.io/object-digital-passport/verify.html).
- Введите Passport ID, вставьте ссылку `odp://...` или перетащите файл **`.odpass`**. Старые имена **`.odp`** эталонный интерфейс не принимает — при том же содержимом ZIP переименуйте в **`.odpass`** или выгрузите пакет заново со страницы паспорта.
...
3. **Распространение** — передаются **Passport ID** (`ODP-...`) и при использовании размещённый **`passport.json`** (или **`dataUrl`**, указывающий на **`.odpass`**).
...
Операторские заметки: [`../../RELEASE_v0.3.md`](../../RELEASE_v0.3.md).
