...
**Контекст реестра для ссылок:** человекочитаемые ссылки протокола (`odp://…`, `odpc://…`; см. §12 и §19) **не** кодируют, в какой **сети** и по какому **адресу контракта** создана запись. Клиент **обязан** сопоставлять **chain + адрес реестра + ABI**, как и в остальной спецификации; один только `odp://` **без** этого контекста **неоднозначен**. См. §19.
...
### 12.1 QR паспорта (нормативный минимум)

Основная нагрузка для объектного QR:
...
- Кодировка: UTF-8
- Fallback HTTPS (задаётся реализацией), напр.: `https://verify.example.com/ODP-2026-03-004829301`

**Authority** в смысле RFC 3986 для схемы `odp` **должен** быть ровно одним **Passport ID** по §2 — без path и query, пока будущая версия спеки не определит иное.

### 12.2 QR профиля / creator (`odpc`, provisional)

Эталонный UI кодирует **Profile ID** (§3) для QR эмитента, напр.:

```
odpc://P-482-930-174-005
```

Имя схемы `odpc` — **provisional** до стабильной **v1**; семантика: **authority** = строка **Profile ID**. Иерархические path (`/passports`, `/proofs`) и query **зарезервированы / экспериментальны** — до нормативного определения реализации **не должны** на них опираться для интеропа.

### 12.3 Контекст реестра (нормативно)

Ни **`odp://`**, ни **`odpc://`** не указывают **chain ID** и **адрес реестра**. Любой клиент, читающий цепочку по этим URI, **обязан** получить **контекст реестра** (`chainId`, адрес основного контракта, ABI, при необходимости спутники) из конфигурации, доверенного deeplink или доверенного резолвера (§19). См. также §7.
...
Минимальный DID document (JSON): `id`, `verificationMethod` на Ethereum-адрес создателя (или отдельные ключи для VC), `alsoKnownAs` к `passport.json` / деплою. Глобальный on-chain resolver в v0.3 не требуется. Для **уровня URI** (`odp://`, `odpc://`), контекста реестра и опциональных HTTP-профилей см. **§19**.
...
### 19.1 Нормативные URI-схемы (минимум для QR и deeplink)

| Схема | Authority (токен в стиле host) | Правило |
|--------|-------------------------------|---------|
| `odp` | `<Passport ID>` строго по §2 | **Должен** совпадать с §2; QR по умолчанию — §12.1 |
| `odpc` | `<Profile ID>` строго по §3 | **Provisional** до стабильной v1; эталонный UI — §12.2 |
...
Чтобы связать `odp://…` или `odpc://…` с on-chain состоянием, нужен **контекст реестра**: как минимум **`chainId`**, адрес основного реестра и **ABI** (плюс опционально спутники). Клиент **обязан** получать его из: конфигурации пользователя/интегратора; доверенного транспорта (HTTPS-страница с метаданными); ответа **доверенного** off-chain резолвера (§19.4). URI **без** метаданных реестра **неоднозначен** — возможны несовместимые реестры на разных адресах.

### 19.3 Связь с `did:odp` (§18.2)

**URI-схемы** (`odp://`, `odpc://`) удобны для QR и вставки. Строки **`did:odp:passport:`** / **`did:odp:profile:`** — форма **W3C DID** для документов и VC. Один и тот же Passport ID / Profile ID может фигурировать в обоих слоях; DID-документ по-прежнему **опционален** (§18.2).
...
| Голая ссылка `odp://` / `odpc://` **без** метаданных реестра | **Недостаточно**, чтобы знать, какой реестр опрашивать |
