...
**Переносимый контейнер (нормативное расширение):** **`.odpass`** (ZIP). **Историческая справка:** раньше встречалось расширение **`.odp`** для того же содержимого; **эталонные верификаторы в этом репозитории принимают только `.odpass`** (переименуйте или выгрузите заново). Сторонние верификаторы **могут** по своему усмотрению принимать и `.odp`, но **примеры и тесты совместимости — только `.odpass`**.
...
Полный:    C-482-930-174-005 / 0x742d35Cc…4438f44e
```

**Полная** строка для публичных материалов — короткий ID и **полный** адрес кошелька (желательно checksum EIP-55). Человекочитаемое **имя** в маркетинговых текстах можно ставить до или после адреса, но оно **не** часть канонической строки и **не** проверяется on-chain.
...
Музей на museum.com:
  ...
  Полный:    M-204-839-112-441 / 0xB3F924ee…1823A3c8A
```
...
**Нормативный** переносимый файл — **`.odpass`**: ZIP-контейнер для распространения и резервного копирования ODP-паспорта. **Историческая справка:** расширение **`.odp`** встречалось для того же содержимого; **эталонные инструменты и верификаторы в этом репозитории принимают только `.odpass`** (переименуйте или выгрузите заново).
