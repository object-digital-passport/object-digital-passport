...
**Историческая справка (до v0.4):** в ранних черновиках для профильного QR была отдельная схема **`odpc://`**. На практике она не использовалась и **не** совместима с единым правилом **`odp://`** здесь. Реализации **не должны** выдавать **`odpc://`**.
...
Минимальный DID document (JSON): `id`, `verificationMethod` на Ethereum-адрес создателя (или отдельные ключи для VC), `alsoKnownAs` к `passport.json` / деплою. Глобальный on-chain resolver в v0.3 не требуется. Для **уровня URI** (`odp://`), контекста реестра и опциональных HTTP-профилей см. **§19**.
...
**Про аналогию с DNS:** ниже используется **образ DNS** (человекочитаемое имя → «разрешение» в данные), только для ясности. ODP **не** DNS: нет единого корня вроде ICANN. **Источник истины** — выбранный вами деплой в блокчейне (**chain ID** + **адрес контракта** + ABI). Далее нормируются **строковые формы** и **опциональные** off-chain помощники — не глобальная иерархия имён.
...
Черновики **до v0.4** описывали **`odpc://`** только для профиля; в реальности не применялось и с единым **`odp://`** **не** совместимо. Не выдавать **`odpc://`**.
...
- `GET …/passport/{humanId}` → JSON с данными из `getPassport`, `getCreator`, списками proof и явными полями `chainId`, `registryAddress`, пробой `CONTRACT_VERSION`, опционально спутники.
...
- **Контент:** загрузка байтов `passport.json` (напрямую с `dataUrl` или через прокси резолвера) **всё равно** сверяется с on-chain **`dataHash`** — резолвер может отдать неверные байты.
