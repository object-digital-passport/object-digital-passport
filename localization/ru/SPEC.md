...
- **Этот документ описывает эталонную линию v0.3** (поколение на цепочке **3**). Другие поколения — отдельные реестры с другим байткодом и ABI; интеграции **обязаны** сопоставлять **сеть + адрес + `CONTRACT_VERSION`** с правильным ABI — без доработок код не взаимозаменяем.
...
### Мульти-контрактная архитектура v0.3 (кратко, нормативный смысл)

Эталонный стек **v0.3** **разделён на несколько контрактов**:

- **`ObjectDigitalPassport.sol`** — **основной реестр** (паспорта, профили, доказательства, управление в рамках ABI).
- **`ODPPassportLib.sol`** — **отдельно задеплоенная линкуемая библиотека** с тяжёлой **pure**-логикой (проверки минта, кодирование), чтобы реестр уложился в лимит **24 КиБ (EIP-170)**; общий **`error EC`** с реестром.
- **`ODPWalletDocumentAnchor.sol`** (опциональный **спутник**) — операции **`attestExternalDocument`** / чтение якорей вынесены из основного реестра в v0.3+; конструктор спутника получает адрес основного реестра и проверяет регистрацию через него.

Порядок деплоя: **сначала библиотека**, затем **реестр** (с метаданными линковки), затем **спутник** при необходимости — см. скрипты в репозитории.
...
### v0.3 on-chain — обзор возможностей
...
**Убрано из референсного байткода v0.3 `ObjectDigitalPassport` (EIP-170):** on-chain **counterfeit concern** (`raiseCounterfeitConcern` / `clearCounterfeitConcern` / `getCounterfeitConcern`). На **старых** деплойментах эти точки входа могут сохраняться; референсный **Passport** UI скрывает блоки counterfeit, если в ABI их нет. **Продуктовые формулировки** о спорах или претензиях к подлинности **могут** жить в **`passport.json`**, сопутствующих материалах и метаданных **`submitProof`** — это **оффчейн / запись proof**, а не глобальный on-chain флаг в v0.3. **Дорожная карта (справочно):** в **будущей** версии спецификации или деплое **может** быть снова введён или заменён такой on-chain механизм; для эталонного байткода **v0.3** в этом репозитории он **вне области** — см. **`RELEASE_v0.3.md`**.
