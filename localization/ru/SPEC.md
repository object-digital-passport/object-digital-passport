...
- **С v0.1 обратной совместимости нет** ни для Creator ID, ни для паспортных записей: v0.1 и v0.2 нужно считать разными реестрами.
...
| **Deployment (деплоймент)** | Один конкретный экземпляр смарт-контракта по одному адресу (один реестр). Creator IDs и паспортные записи привязаны к конкретному деплойменту. |
...
- Короткий Creator ID (**обязательно**)
...
должны публично публиковать короткий Creator ID вместе с полным адресом кошелька.**
...
| `prover` | `string` | да | Creator ID институции (например, `P-029-384-751-224`) |
...
| Human ID | Полный идентификатор объекта в человекочитаемом виде | `ODP-2026-03-004829301` |
...
- Creator ID текстом (только справочно; не источник доверия)
- Название объекта
- Имя создателя
- Год создания
- Номер издания
...
**Нота о доверии (нормативно):** Creator ID, напечатанный на упаковке/лейбле, сам по себе **не является** доверенным источником и **не обязателен** на метке. Проверяющим следует получать Creator ID из верифицированных публичных источников (официальный сайт и публичные профили в соцсетях), после чего сверять с данными on-chain.
...
2. Пересчитать `localDataHash` как SHA-256 от канонического JSON ODP (те же правила канонизации, что и у протокольного верификатора),
   используя нормализацию `humanId` для сравнения входа для on-chain хэша (т.е. при пересчёте считать humanId из бандла как `humanId: null`).
3. Сравнить `localDataHash` с on-chain `dataHash` для указанного `Human ID`.
...
- Человекочитаемые имена — Creator ID это номер, а не имя
