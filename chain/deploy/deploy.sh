...
#
# Usage (from repo root (`npm run compile` or `cd chain && npm run compile`) or from chain/deploy/):
...
if [[ ! -f "$SCRIPT_DIR/user-setup/private.local.env" && ! -f "$SCRIPT_DIR/.env" ]]; then
  ...
  echo "  chain/deploy/user-setup/private.local.env  (recommended: cp chain/deploy/user-setup/private.local.env.example)" >&2
  echo "  or chain/deploy/.env with PRIVATE_KEY (see chain/deploy/user-setup/README.md)" >&2
  exit 1
fi
...
npx hardhat run chain/deploy/scripts/deploy.js --network "$NETWORK"

echo "Done. Addresses and abi: chain/deploy/deployments/"
