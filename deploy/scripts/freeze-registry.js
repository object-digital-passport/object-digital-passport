  ...
  return Promise.race([
    ...
    new Promise(function (_, reject) {
      setTimeout(function () {
        reject(
          new Error(
            `${label} timed out after ${ms / 1000}s. Check network, or set ODP_POLYGON_RPC_URL to another Polygon HTTPS endpoint in deploy/.env.`
          )
        );
      }, ms);
    }),
...
async function main() {
  ...
  if (!deployer) {
    console.error("No deployer account (PRIVATE_KEY in deploy/user-setup/private.local.env).");
    process.exit(1);
  }
