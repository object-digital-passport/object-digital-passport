...
2. **Форма контракта** (реализация, не протокол): жить ли поверхности экземпляров в ядре v0.7 или в парном спутнике, с учётом бюджета EIP-170 — см. [`EIP170_STRATEGY.md`](../../../../docs/EIP170_STRATEGY.md).
3. **Ссылка исправленного выпуска на бракованный.** В §7a сказано, что после закрытия окна лечение — новый выпуск плюс объявление, указывающее на него. Должен ли этот указатель быть структурным полем, а не свободным текстом внутри объявления, пока не решено.
