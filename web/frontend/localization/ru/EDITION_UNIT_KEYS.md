...
**Состояние референсной реализации.** `chain/contracts/ODPEditionUnits.sol` реализует `openEdition`, `activate` и чтения; три зацепки — в `ObjectDigitalPassport.sol`, который теперь минтит `CONTRACT_VERSION = 7`. Ленивый минт паспорта экземпляра из §6 **описан, но не реализован**: ядро выводит принципала минта из зарегистрированного профиля, а подпись ключом экземпляра профилем не является — нужна четвёртая зацепка, минт, чьё полномочие есть подпись над `(выпуск, номер, владелец)`, проверенная против корня выпуска. Почему прецедент с роутером расширений сюда не переносится — в [ADR-0009](../../../../docs/adr/0009-unit-passport-minting-needs-a-fourth-core-hook.md). Всё, что нужно покупателю до покупки, работает и без него.
...
На уровне протокола — нет. Осталось писать код: спутник и три зацепки в ядре выше, а затем перевод §§1–19 `SPEC.md` с v0.6 на v0.7, когда контракт появится и адреса станут настоящими.
