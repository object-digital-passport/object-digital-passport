...
*Проектная записка для линии **v0.7**. **Ненормативная** — фиксирует принятые решения и их обоснование. Обязательные правила появятся в [`SPEC.md`](../../../../SPEC.md), когда модель устоится.*
...
Это функция для профиля **B** (бренд). Для C / P / M ничего не меняется.
...
- **Для покупателя бесплатно.** Ни кошелька, ни криптовалюты, ни аккаунта.
...
**Владельцем изначально становится адрес ключа экземпляра.** Код ведёт себя как документ на предъявителя: у кого бумажка — тот и владеет. Это буквально повторяет физику кода, лежащего в коробке, которая путешествует вместе с вещью. Потерял коробку — потерял паспорт.

**Позже одним нажатием паспорт переводится на настоящий кошелёк**: владелец подписывает передачу тем же ключом. На входе — ни одного барьера, а тому, кому вещь важна, доступна полноценная гарантия владения.

Кто платит за ленивый минт — решение коммерческое, а не протокольное. Рабочий вариант по умолчанию: бренд оплачивает первый год после дропа как маркетинговый расход, дальше — владелец.
...
1. **Где живёт контракт.** Новый спутник (`ODPEditionUnits`), читающий основной реестр, против изменений в ядре. И паттерн спутников, и бюджет EIP-170 говорят в пользу спутника — см. [`EIP170_STRATEGY.md`](../../../../docs/EIP170_STRATEGY.md).
2. **Биты маски анкоров** для `unit_key_set` и `unit_variant_commit` в `anchorTypesMask`.
3. **Раздача соли** для обязательства о варианте: кто передаёт покупателю `salt_i` и что происходит, когда бренда больше нет.
4. **Бренды без GTIN.** Путь через GS1 предполагает его наличие; у мелких брендов его может не быть.
5. **Экономика релея** — спам и гриферство на бесплатной точке активации, открытой для всех.
6. **Отзыв выпуска** — рекол, брак печати, скомпрометированная партия кодов.
7. **Локализация** алфавита печатаемого кода и контрольных знаков.
