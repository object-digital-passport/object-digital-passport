...
| `shippingDate` | опционально | **Заявленный план**, нужен только для проверки аномалий в §8.3. Гейтом ничему намеренно не служит — см. §7a |
...
**Окно отзыва, которое закрывается по фактам, а не по датам.** Отозвать выпуск можно, пока верно *и то и другое*: ни один экземпляр не активирован и бренд не объявил отгрузку. Любое из двух закрывает дверь навсегда и для всех, включая `governance`. Внутри окна опечатка, пойманная на складе, чинится обычным способом и никому не вредит; вне окна стереть уже ничего нельзя.

Гейтом намеренно **не** служит `shippingDate` из анкора. Это план, заявленный при минте, а анкоры неизменяемы — значит перенос производства на два месяца либо захлопнет дверь, когда ещё ничего не отгружено, либо оставит её открытой, когда коробки уже в магазинах, и единственным лечением будет перевыпуск паспорта из-за сдвига логистики. План — не факт. Факты здесь два: кто-то стёр код, или бренд сказал «мы отгружаем».

**Отгрузка — собственное необратимое заявление бренда**, публикуется в момент реальной отгрузки. Перенос означает просто «опубликуй позже». А «можно вообще не публиковать» — лазейка небольшая: первый же покупатель со скретчем закрывает дверь сам, а до тех пор каждая проверка показывает, что окно открыто. Бренд, чей товар лежит в магазинах, пока реестр объявляет живое право стереть записи покупателей, показывает это самим покупателям.
...
| 12 | Отзыв выпуска | **Убран.** Отзыв остаётся только внутри окна, которое закрывается первой активацией или объявлением отгрузки — навсегда и для всех, включая `governance` |
| 13 | Чем закрывается окно | Наблюдаемыми фактами, но не заявленным `shippingDate`: неизменяемый анкор не умеет следовать за съехавшей датой производства |
| 14 | Как сказать, что что-то не так, потом | Append-only объявление о выпуске, показываемое на выпуске **и** на каждом паспорте экземпляра под ним, и никогда не вердикт об экземпляре |

## 11. Открытые вопросы

1. **Бренды без GTIN.** Путь через GS1 предполагает его наличие; у мелких брендов его может не быть, а запасной ODP-путь теряет ту самую совместимость с EU DPP, ради которой §3.5 и затевался.
2. **Экономика релея** — спам и гриферство на бесплатной точке активации, открытой для всех. Тот, кто держит релей, платит за всё, что ему принесут.
3. **Локализация** алфавита печатаемого кода и контрольных знаков.
4. **Форма контракта** (реализация, не протокол): жить ли поверхности экземпляров в ядре v0.7 или в парном спутнике, с учётом бюджета EIP-170 — см. [`EIP170_STRATEGY.md`](../../../../docs/EIP170_STRATEGY.md).
5. **Ссылка исправленного выпуска на бракованный.** В §7a сказано, что после закрытия окна лечение — новый выпуск плюс объявление, указывающее на него. Должен ли этот указатель быть структурным полем, а не свободным текстом внутри объявления, пока не решено.
