...
- **Format:** ZIP with extension `.odpass` (legacy `.odp`); required `passport.json` + `manifest.json`; sidecar bytes under `originals/` with paths in `manifest.originals` (v0.3); legacy `original/*` / `image/*` — see **SPEC §15**.
