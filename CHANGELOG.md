...
- **§15.3 says what an offline check actually establishes, and the core-principles list stops overselling it.** "Authenticity can be verified without internet using only hashes" was not true: §15.2 requires comparing `localDataHash` against the **on-chain** `dataHash`, so with no chain a verifier can confirm a bundle is internally consistent and nothing more — not that the document was registered, by whom, whether the passport was revoked or its issuer stopped, or who holds it now. A forger can produce an internally consistent bundle. Such a verifier now MUST show the result as unconfirmed and MUST NOT show an assurance tier (§11). The same section makes two more things normative: no verifier may rest on a single RPC endpoint, and *could not reach the registry* must never be displayed as *invalid*. §7 links the dependency from where the single network is declared, and states plainly that if Polygon PoS ends, live verification ends with it — there is no fallback anchor in this line.
- **A profile can be stopped, and a compromise can be dated — in two different places, on purpose.** §17 previously left a profile valid forever after a key was lost or stolen: no rotation, no revocation, nothing but a post asking people to believe a different number. `revokeCreator()` now lets the owning wallet stop its own profile — irreversible, no arguments, future issuance only, and already-minted passports untouched. The call takes no date deliberately: whoever holds the key can make it, a thief included, so a caller-chosen date would let a thief name the profile's registration day and discredit the issuer's whole history in one transaction. Dating a compromise therefore lives where a stolen key cannot reach — a `compromised` array in the issuer's own `/.well-known/odp.json` (§3.1), carrying `profileId`, `since`, and optionally `replacedBy`. §11 says what a verifier does with both: show that the profile was stopped, never present the stop as a verdict on the object, and where a statement exists, say which side of `since` the record falls on. What is still not solved is stated plainly in §17 — a *lost* key cannot even be stopped.
